...
export function AdminPage({ onboarding: startOnboarding }) {
  const { slug }    = useParams()
  const navigate    = useNavigate()
  const { user, logout, updatePassword } = useAuth()
  const { studio, loadStudio, createStudio } = useStudio(user)
  ...
  useEffect(() => {
    if (!user) return
    loadStudio().then(s => {
      if (!s && !startOnboarding) setOnboarding(true)
      // Redirect if slug doesn't match
      if (s?.slug && slug && s.slug !== slug) navigate(`/${s.slug}/admin`, { replace: true })
    })
  }, [user])

  // Step 2: load config + event once studio is known
  useEffect(() => {
    if (!studio?.id) return
  ...
  // Realtime
  useEffect(() => {
    return bookingsHook.subscribeRealtime(b => showToast(`Novo agendamento: ${b.name}`))
  }, [eventHook.event?.id])
  ...
  const handleOnboardingFinish = async (form) => {
    ...
    setOnboarding(false)
    setAppReady(true)
    navigate(`/${s.slug}/admin`, { replace: true })
    showToast("Bem-vindo! Configure seu evento nas Configuracoes.")
  }

  // ── Booking handlers ──
  const handleConfirmSinal = async (id) => {
    await bookingsHook.confirmSinal(id); showToast("Sinal confirmado!")
  }
  const handleOpenSession = (booking, edit=false) => {
    setSessionBooking(booking); setIsEditSession(edit)
  }
  ...
  const handleSaveEdit = async (id, updates) => {
    await bookingsHook.updateBooking(id, updates); setEditBooking(null); showToast("Agendamento atualizado!")
  }
  const handleDeleteBooking = async (id) => {
    await bookingsHook.deleteBooking(id); setConfirmDeleteId(null); showToast("Agendamento removido.")
  }
    ...
    <>
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <a
              ...
              style={{ background:"transparent", border:"none", color:T.textMuted, fontSize:12, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", textDecoration:"none" }}
            >
              Ver site
            </a>
        ...
        {adminTab==="doacoes" && config.donationConfig?.enabled && (
          <DonationsTab
            donations={donationsHook.donations} bookings={bookingsHook.bookings}
            donationConfig={config.donationConfig} onAddDonation={donationsHook.addDonation}
          />
        )}
        {adminTab==="resumo" && (
          <SummaryTab
      ...
      {editBooking && (
        <EditModal
          ...
          onSave={handleSaveEdit} onClose={()=>setEditBooking(null)}
      ...
      {sessionBooking && (
        <SessionModal
          ...
          onSave={handleSaveSession} onClose={()=>setSessionBooking(null)}
